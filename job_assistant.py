...
AI_DIFFERENTIATOR_RULE = (
    ...
    "there.\n"
    "CRITICAL — DO NOT INFLATE THE DEGREE OF THIS EXPERIENCE: this is a genuine but "
    "self-directed personal learning project, not professional/production experience "
    "or a claim of expert-level mastery. NEVER describe it using inflating language "
    "like 'proven experience', 'advanced AI systems', 'sophisticated AI systems', "
    "'extensive experience', or similar — these overstate what a self-directed side "
    "project actually represents, even though every individual technical fact in the "
    "description is true. Describe it factually and modestly instead: state what was "
    "specifically built (e.g. 'built a self-hosted RAG chatbot using LangGraph and "
    "ChromaDB' or 'developed a multi-agent automation pipeline with CrewAI') without "
    "a grandiose adjective in front of it. The specific tools and what was built are "
    "impressive enough on their own — do not editorialize on top of them."
)
...
WRITING_STYLE_RULE = (
    ...
    "AVOID INFLATING ADJECTIVES: watch for words that make an accurate underlying fact "
    "sound more impressive than it actually is — 'proven', 'advanced', 'sophisticated', "
    "'extensive', 'expert-level' are the most common offenders. Every individual fact "
    "can be true while the overall impression is still misleading if wrapped in "
    "grandiose language. Describe what was actually done, plainly — the real tools and "
    "real outcomes carry their own weight without an inflating adjective in front of "
    "them."
)
...
def build_crew(job_posting_text: str, candidate_background_text: str) -> Crew:
    analyze_posting = Task(
        description=(
            ...
            f"(7) LANGUAGE REQUIREMENT SEVERITY — the candidate is a fluent, "
            f"professional-level English speaker, so an English proficiency "
            f"requirement is NEVER a concern and must always be classified as "
            f"'none', regardless of how it's phrased in the posting (e.g. "
            f"'fluent English required', 'excellent written and verbal English' — "
            f"none of these count, since the candidate already meets them). This "
            f"check exists specifically to catch requirements for languages OTHER "
            f"than English — most commonly German, given the posting may be in "
            f"German or explicitly require German proficiency. Classify the "
            f"severity of any NON-ENGLISH language requirement you find using "
            f"these categories:\n"
            f"- 'none' — no non-English language requirement mentioned at all "
            f"(this includes postings that only require English).\n"
            ...
            f"where the language is listed as a plus while English is the actually "
            f"mandatory one.\n"
            ...
            f"und Schrift' — fluent, written and spoken required\"). If the posting "
            f"only requires English, or mentions no language requirement at all, set "
            f"language_requirement_detail to 'No language requirement mentioned.'\n\n"
